...
static int i_update_FW(void)
{
	...
	if (0x11 == private_ts->vendor_sensor_id)
	{
		...
		printk("himax, tpd_probe, hx8527e_ctp_module_name = TRULY\n");
	}
	else if(0x12 == private_ts->vendor_sensor_id)
	{
		ImageBuffer= i_CTPM_FW_YUSHUN;
		fullFileLength= sizeof(i_CTPM_FW_YUSHUN);
		printk("himax, tpd_probe, hx8527e_ctp_module_name = YUSHUN\n");
	}
	else
	{
		printk("himax No compared Sensor ID , stop update firmware .\n");
		return 0;
	}

	pr_info("0x%x, 0x%x, 0x%x\n",
		(u32)private_ts->vendor_config_ver,
		FW_CFG_VER_FLASH_ADDR,
		* (ImageBuffer + FW_CFG_VER_FLASH_ADDR));
	checksumResult = himax_calculateChecksum(true);
	if ( (private_ts->vendor_config_ver < * (ImageBuffer+ FW_CFG_VER_FLASH_ADDR)) || (checksumResult == 0) )
		{
			if(fts_ctpm_fw_upgrade_with_sys_fs(ImageBuffer,fullFileLength,true) == 0)
				printk("%s: TP upgrade error\n", __func__);
			else
				I("%s: TP upgrade OK\n", __func__);
#ifdef HX_RST_PIN_FUNC
			himax_HW_reset(false, true);
...
static void himax_read_TP_info(struct i2c_client *client)
{
	...
	void ESD_HW_REST(void)
	{
		...
		if (atomic_read(&private_ts->suspended) && private_ts->SMWP_enable) {
			buf[0] = 0x8F;
			buf[1] = 0x20;
...
//		printk("%s buf[%d]=%x\n", __func__, i, buf[i]);
	...
	if((buf[0]==0x80)&&(buf[1]==0x80)&&(buf[2]==0x80)&&(buf[3]==0x80)&&(buf[4]==0x80)&&(buf[5]==0x80)&&(buf[6]==0x80))
		{
			I("%s: WAKE UP system!\n", __func__);
...
static ssize_t himax_gesture_proc_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
	...
	//printk("%s %d SMWP_enable = %d\n",__func__,__LINE__,ts->SMWP_enable);
...
static int himax852xes_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(is_tp_driver_loaded) {
		E("%s: other driver has been loaded\n", __func__);
		return -ENODEV;
	}
...
//	enable_irq_wake(client->irq);
	...
	if (IS_ERR_OR_NULL(gesture_proc))
		printk("%s create_proc_entry gesture_proc failed\n", __func__);
#endif
